...
public class Loot : MonoBehaviour
{
	...
	void Start()
	{
		...
		{
			Debug.LogError($"Loot {gameObject.name} requires at least 2 BoxCollider2Ds.");
			return;
		}

		// Assign the first collider to `pickUpCollision` and the second to `triggerCollision`
		pickUpCollision = colliders[0];
		triggerCollision = colliders[1];

		// Make both colliders triggers as per your logic
		pickUpCollision.isTrigger = true;
